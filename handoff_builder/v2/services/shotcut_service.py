...
from ..plans.semantic import ValidatedPreviewPlan, load_and_validate_preview_plan
...
def _load_job_context(workspace_root: Path, render_job_id: str) -> _JobContext:
    ...
    try:
        ...
        return _JobContext(
            ...
            paths=_artifact_paths(report_path.parent),
        )
    finally:
        connection.close()


def _artifact_paths(output_dir: Path) -> ShotcutServicePaths:
