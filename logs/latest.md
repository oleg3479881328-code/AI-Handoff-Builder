...
- Validation on Sunday, July 26, 2026:
  ...
  - compile validation:
    ...
    - `python -m compileall handoff_builder app.py` -> success
# 2026-08-02 - Compact owner-facing controls

- Changed `Prepare Handoff (v1)` so empty visual panels no longer dominate the screen:
  - source list is hidden until materials are added;
  - MASTER_AUDIO metrics are behind `Показать статус MASTER_AUDIO`;
  - the empty event log is behind `Показать журнал`;
  - existing preparation, import, validation, final ZIP, and folder buttons remain available.
- Validation:
  - `python -m pytest -q tests\\test_app_v2_ui.py tests\\test_v2_master_audio_workflow.py` -> `11 passed, 1 skipped`
  - `python -m py_compile app.py` -> success
  - rebuilt `dist\\V0.1.0_AI_Handoff_Builder\\V0.1.0_AI_Handoff_Builder.exe`
